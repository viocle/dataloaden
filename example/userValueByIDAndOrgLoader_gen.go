...
func (r *UserValueByIDAndOrgLoaderRedisConfig) Context() (context.Context, context.CancelFunc) {
...
type userValueByIDAndOrgLoaderBatch struct {
	loader    *UserValueByIDAndOrgLoader
	now       int64
	done      chan struct{}
...
func (l *UserValueByIDAndOrgLoader) Reset() {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *UserValueByIDAndOrgLoader) unsafeBatchSet() {
	if l.batch == nil {
		...
		l.batch = &userValueByIDAndOrgLoaderBatch{loader: l, now: 0, done: make(chan struct{}), keysMap: b.keysMap, keys: b.keys[:0], data: nil, errors: nil, reqCount: 0, checkedIn: 0, lock: sync.Mutex{}}
...
func (l *UserValueByIDAndOrgLoader) createNewBatch() *userValueByIDAndOrgLoaderBatch {
	return &userValueByIDAndOrgLoaderBatch{
		loader:    l,
		now:       0,
...
func (l *UserValueByIDAndOrgLoader) LoadThunk(key UserByIDAndOrg) (User, func() (User, error)) {
	if l.redisConfig != nil {
		...
		defer cancel()
		v, err := l.redisConfig.GetFunc(ctx, UserValueByIDAndOrgLoaderCacheKeyPrefix+l.redisConfig.KeyToStringFunc(key))
		if err == nil {
...
func (l *UserValueByIDAndOrgLoader) unsafeAddToBatchNoLock(key UserByIDAndOrg) (User, func() (User, error)) {
	l.unsafeBatchSet()
	batch := l.batch
...
func (l *UserValueByIDAndOrgLoader) redisPrime(key UserByIDAndOrg, value User) bool {
	ctx, cancel := l.redisConfig.Context()
	defer cancel()
	if err := l.redisConfig.SetFunc(ctx, UserValueByIDAndOrgLoaderCacheKeyPrefix+l.redisConfig.KeyToStringFunc(key), value, l.redisConfig.SetTTL); err != nil {
		return false
...
func (l *UserValueByIDAndOrgLoader) PrimeMany(keys []UserByIDAndOrg, values []User) []bool {
	...
	if l.redisConfig != nil {
		...
		if l.redisConfig.SetManyFunc != nil && len(keys) > 1 {
			...
			for i, key := range keys {
				kSet[i] = UserValueByIDAndOrgLoaderCacheKeyPrefix + l.redisConfig.KeyToStringFunc(key)
			}
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
...
func (b *userValueByIDAndOrgLoaderBatch) getResult(pos int) (User, error) {
	var data User
	b.lock.Lock()
	if pos < len(b.data) {
		data = b.data[pos]
