...
type UserLoaderConfig struct {
	...
	HookExternalCacheGet func(key string) (*example.User, bool)

	// HookExternalCacheSet is a method that provides the ability to set a key in an external cache with an external hook.
	// This replaces the use of the internal cache.
	HookExternalCacheSet func(key string, value *example.User) error
	...
	HookAfterPrimeMany func(keys []string, values []*example.User)

	// HookAfterClear is called after a value is cleared from the cache
	HookAfterClear func(key string) error
...
func NewUserLoader(config UserLoaderConfig) *UserLoader {
	...
	if config.RedisConfig != nil {
		...
		if l.redisConfig.GetFunc != nil && l.redisConfig.SetFunc != nil && l.redisConfig.DeleteFunc != nil {
			...
			l.batchResultSet = func(key string, value *example.User) {
...
func (r *UserLoaderRedisConfig) Context() (context.Context, context.CancelFunc) {
...
type UserLoader struct {
	...
	batchResultSet func(string, *example.User)
...
func (l *UserLoader) Reset() {
	...
	if l.redisConfig == nil {
		...
		l.cacheExpire = make(map[string]*UserLoaderCacheItem)
...
func (l *UserLoader) LoadThunk(key string) (*example.User, func() (*example.User, error)) {
	if l.redisConfig != nil {
		// using Redis
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		v, err := l.redisConfig.GetFunc(ctx, UserLoaderCacheKeyPrefix+key)
		if err == nil {
			if v == "" || v == "null" {
...
func (l *UserLoader) unsafeAddToBatchNoLock(key string) (*example.User, func() (*example.User, error)) {
	l.unsafeBatchSet()
	batch := l.batch
...
func (l *UserLoader) unsafeAddToBatch(key string) (*example.User, func() (*example.User, error)) {
...
func (l *UserLoader) redisPrime(key string, value *example.User) bool {
	ctx, cancel := l.redisConfig.Context()
	defer cancel()
	if err := l.redisConfig.SetFunc(ctx, UserLoaderCacheKeyPrefix+key, value, l.redisConfig.SetTTL); err != nil {
		return false
...
func (l *UserLoader) unsafePrime(key string, value *example.User, forceReplace bool) bool {
	if l.redisConfig != nil {
		// using Redis
...
func (l *UserLoader) PrimeManyNoReturn(keys []string, values []*example.User) {
...
func (l *UserLoader) ForcePrime(key string, value *example.User) {
...
func (l *UserLoader) Clear(key string) error {
	if l.redisConfig != nil {
		// using Redis
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		err := l.redisConfig.DeleteFunc(ctx, UserLoaderCacheKeyPrefix+key)
		if err != nil {
			return err
		}
...
func (l *UserLoader) ClearAllPrefix(prefix string) error {
	if l.redisConfig != nil {
		// using Redis
		if l.redisConfig.GetKeysFunc != nil {
			...
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
			keys, err := l.redisConfig.GetKeysFunc(ctx, UserLoaderCacheKeyPrefix+prefix+"*")
...
func (l *UserLoader) ClearAll() error {
