...
func (r *StringLoaderRedisConfig) Context() (context.Context, context.CancelFunc) {
...
func (l *StringLoader) Reset() {
	...
	if l.redisConfig == nil {
		...
		l.cacheExpire = make(map[string]*StringLoaderCacheItem)
...
func (l *StringLoader) LoadThunk(key string) (string, func() (string, error)) {
	if l.redisConfig != nil {
		...
		defer cancel()
		v, err := l.redisConfig.GetFunc(ctx, StringLoaderCacheKeyPrefix+key)
		if err == nil {
			return v, nil
...
func (l *StringLoader) redisPrime(key string, value string) bool {
	ctx, cancel := l.redisConfig.Context()
	defer cancel()
	if err := l.redisConfig.SetFunc(ctx, StringLoaderCacheKeyPrefix+key, value, l.redisConfig.SetTTL); err != nil {
		return false
...
func (l *StringLoader) Clear(key string) error {
	if l.redisConfig != nil {
		// using Redis
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		err := l.redisConfig.DeleteFunc(ctx, StringLoaderCacheKeyPrefix+key)
		if err != nil {
			return err
		}
...
func (l *StringLoader) ClearAllPrefix(prefix string) error {
	if l.redisConfig != nil {
		// using Redis
		if l.redisConfig.GetKeysFunc != nil {
			...
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
			keys, err := l.redisConfig.GetKeysFunc(ctx, StringLoaderCacheKeyPrefix+prefix+"*")
...
func (l *StringLoader) ClearAll() error {
