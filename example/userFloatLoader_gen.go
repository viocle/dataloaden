...
func (r *UserFloatLoaderRedisConfig) Context() (context.Context, context.CancelFunc) {
...
func (l *UserFloatLoader) Reset() {
	...
	if l.redisConfig == nil {
		...
		l.cacheExpire = make(map[float64]*UserFloatLoaderCacheItem)
...
func (l *UserFloatLoader) LoadThunk(key float64) (*User, func() (*User, error)) {
	if l.redisConfig != nil {
		...
		defer cancel()
		v, err := l.redisConfig.GetFunc(ctx, UserFloatLoaderCacheKeyPrefix+strconv.FormatFloat(key, 'f', -1, 64))
		if err == nil {
...
func (l *UserFloatLoader) unsafeAddToBatchNoLock(key float64) (*User, func() (*User, error)) {
	l.unsafeBatchSet()
	batch := l.batch
...
func (l *UserFloatLoader) redisPrime(key float64, value *User) bool {
	ctx, cancel := l.redisConfig.Context()
	defer cancel()
	if err := l.redisConfig.SetFunc(ctx, UserFloatLoaderCacheKeyPrefix+strconv.FormatFloat(key, 'f', -1, 64), value, l.redisConfig.SetTTL); err != nil {
		return false
...
func (l *UserFloatLoader) Clear(key float64) error {
	if l.redisConfig != nil {
		// using Redis
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		err := l.redisConfig.DeleteFunc(ctx, UserFloatLoaderCacheKeyPrefix+strconv.FormatFloat(key, 'f', -1, 64))
		if err != nil {
			return err
		}
...
func (l *UserFloatLoader) ClearAllPrefix(prefix string) error {
	if l.redisConfig != nil {
		// using Redis
		if l.redisConfig.GetKeysFunc != nil {
			...
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
			keys, err := l.redisConfig.GetKeysFunc(ctx, UserFloatLoaderCacheKeyPrefix+prefix+"*")
...
func (l *UserFloatLoader) ClearAll() error {
