...
func (l *UserIntLoader) PrimeMany(keys []int, values []*User) []bool {
	...
	if l.redisConfig != nil {
		...
		if l.redisConfig.SetManyFunc != nil && len(keys) > 1 {
			...
			for i, key := range keys {
				kSet[i] = UserIntLoaderCacheKeyPrefix + strconv.FormatInt(int64(key), 10)
			}
...
func (l *UserIntLoader) ClearAllPrefix(prefix string) {
	if l.redisConfig != nil {
		...
		if l.redisConfig.GetKeysFunc != nil {
			...
			keys, _ := l.redisConfig.GetKeysFunc(context.Background(), UserIntLoaderCacheKeyPrefix+prefix+"*")
	...
	if l.redisConfig != nil && len(b.errors) > 0 {
		...
		if len(b.errors) > 1 && l.redisConfig.SetManyFunc != nil {
			...
			for i, key := range b.keys {
				if b.errors[i] == nil {
					// convert keys slice (of int) to string slice
					kSet = append(kSet, UserIntLoaderCacheKeyPrefix+strconv.FormatInt(int64(key), 10))
