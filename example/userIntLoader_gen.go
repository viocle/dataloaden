...
func (l *UserIntLoader) Clear(key int) {
	if l.redisConfig != nil {
		...
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		l.redisConfig.DeleteFunc(ctx, UserIntLoaderCacheKeyPrefix+strconv.Itoa(key))
...
func (l *UserIntLoader) ClearAllPrefix(prefix string) {
	if l.redisConfig != nil {
		// using Redis
		if l.redisConfig.GetKeysFunc != nil {
			...
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
			keys, _ := l.redisConfig.GetKeysFunc(ctx, UserIntLoaderCacheKeyPrefix+prefix+"*")
