	...
	if config.RedisConfig != nil {
		...
		if l.redisConfig.GetFunc != nil && l.redisConfig.SetFunc != nil && l.redisConfig.DeleteFunc != nil {
			...
			l.batchResultSet = func(key int, value *User) {
				l.redisConfig.SetFunc(context.Background(), UserIntLoaderCacheKeyPrefix+strconv.Itoa(key), value, l.redisConfig.SetTTL)
			}
...
func (r *UserIntLoaderRedisConfig) Context() (context.Context, context.CancelFunc) {
...
func (l *UserIntLoader) Reset() {
	...
	if l.redisConfig == nil {
		...
		l.cacheExpire = make(map[int]*UserIntLoaderCacheItem)
...
func (l *UserIntLoader) LoadThunk(key int) (*User, func() (*User, error)) {
	if l.redisConfig != nil {
		...
		defer cancel()
		v, err := l.redisConfig.GetFunc(ctx, UserIntLoaderCacheKeyPrefix+strconv.Itoa(key))
		if err == nil {
...
func (l *UserIntLoader) unsafeAddToBatchNoLock(key int) (*User, func() (*User, error)) {
	l.unsafeBatchSet()
	batch := l.batch
...
func (l *UserIntLoader) unsafeAddToBatch(key int) (*User, func() (*User, error)) {
...
func (l *UserIntLoader) LoadAll(keys []int) ([]*User, []error) {
	...
	if l.redisConfig != nil && l.redisConfig.GetManyFunc != nil {
		...
		for idx, key := range keys {
			rKeys[idx] = UserIntLoaderCacheKeyPrefix + strconv.Itoa(key)
		}
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
...
func (l *UserIntLoader) redisPrime(key int, value *User) bool {
	ctx, cancel := l.redisConfig.Context()
	defer cancel()
	if err := l.redisConfig.SetFunc(ctx, UserIntLoaderCacheKeyPrefix+strconv.Itoa(key), value, l.redisConfig.SetTTL); err != nil {
		return false
...
func (l *UserIntLoader) PrimeManyNoReturn(keys []int, values []*User) {
...
func (l *UserIntLoader) PrimeMany(keys []int, values []*User) []bool {
	...
	if l.redisConfig != nil {
		...
		if l.redisConfig.SetManyFunc != nil && len(keys) > 1 {
			...
			for i, key := range keys {
				kSet[i] = UserIntLoaderCacheKeyPrefix + strconv.Itoa(key)
			}
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
...
func (l *UserIntLoader) Clear(key int) error {
	if l.redisConfig != nil {
		// using Redis
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		err := l.redisConfig.DeleteFunc(ctx, UserIntLoaderCacheKeyPrefix+strconv.Itoa(key))
		if err != nil {
			return err
		}
...
func (l *UserIntLoader) ClearAllPrefix(prefix string) error {
	if l.redisConfig != nil {
		// using Redis
		if l.redisConfig.GetKeysFunc != nil {
			...
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
			keys, err := l.redisConfig.GetKeysFunc(ctx, UserIntLoaderCacheKeyPrefix+prefix+"*")
...
func (l *UserIntLoader) ClearAll() error {
	...
	if l.redisConfig != nil && len(b.data) == len(b.errors) && len(b.data) > 0 {
		...
		if len(b.errors) > 1 && l.redisConfig.SetManyFunc != nil {
			...
			for i, key := range b.keys {
				if b.errors[i] == nil {
					// convert keys slice (of int) to string slice
					kSet = append(kSet, UserIntLoaderCacheKeyPrefix+strconv.Itoa(key))
...
func (b *userIntLoaderBatch) getResult(pos int) (*User, error) {
	var data *User
	b.lock.Lock()
	if pos < len(b.data) {
		data = b.data[pos]
