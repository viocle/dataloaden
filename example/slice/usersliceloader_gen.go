...
func (l *UserSliceLoader) PrimeMany(keys []int, values [][]example.User) []bool {
	...
	if l.redisConfig != nil {
		...
		if l.redisConfig.SetManyFunc != nil && len(keys) > 1 {
			...
			for i, key := range keys {
				kSet[i] = UserSliceLoaderCacheKeyPrefix + strconv.FormatInt(int64(key), 10)
			}
...
func (l *UserSliceLoader) ClearAllPrefix(prefix string) {
	if l.redisConfig != nil {
		...
		if l.redisConfig.GetKeysFunc != nil {
			...
			keys, _ := l.redisConfig.GetKeysFunc(context.Background(), UserSliceLoaderCacheKeyPrefix+prefix+"*")
	...
	if l.redisConfig != nil && len(b.errors) > 0 {
		...
		if len(b.errors) > 1 && l.redisConfig.SetManyFunc != nil {
			...
			for i, key := range b.keys {
				if b.errors[i] == nil {
					// convert keys slice (of int) to string slice
					kSet = append(kSet, UserSliceLoaderCacheKeyPrefix+strconv.FormatInt(int64(key), 10))
