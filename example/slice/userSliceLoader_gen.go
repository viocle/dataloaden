	...
	if config.RedisConfig != nil {
		...
		if l.redisConfig.GetFunc != nil && l.redisConfig.SetFunc != nil && l.redisConfig.DeleteFunc != nil {
			...
			l.batchResultSet = func(key int, value []example.User) {
				l.redisConfig.SetFunc(context.Background(), UserSliceLoaderCacheKeyPrefix+strconv.Itoa(key), value, l.redisConfig.SetTTL)
			}
...
func (r *UserSliceLoaderRedisConfig) Context() (context.Context, context.CancelFunc) {
...
func (l *UserSliceLoader) LoadThunk(key int) ([]example.User, func() ([]example.User, error)) {
	if l.redisConfig != nil {
		...
		defer cancel()
		v, err := l.redisConfig.GetFunc(ctx, UserSliceLoaderCacheKeyPrefix+strconv.Itoa(key))
		if err == nil {
...
func (l *UserSliceLoader) unsafeAddToBatchNoLock(key int) ([]example.User, func() ([]example.User, error)) {
	l.unsafeBatchSet()
	batch := l.batch
...
func (l *UserSliceLoader) unsafeAddToBatch(key int) ([]example.User, func() ([]example.User, error)) {
...
func (l *UserSliceLoader) LoadAll(keys []int) ([][]example.User, []error) {
	...
	if l.redisConfig != nil && l.redisConfig.GetManyFunc != nil {
		...
		for idx, key := range keys {
			rKeys[idx] = UserSliceLoaderCacheKeyPrefix + strconv.Itoa(key)
		}
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
...
func (l *UserSliceLoader) redisPrime(key int, value []example.User) bool {
	ctx, cancel := l.redisConfig.Context()
	defer cancel()
	if err := l.redisConfig.SetFunc(ctx, UserSliceLoaderCacheKeyPrefix+strconv.Itoa(key), value, l.redisConfig.SetTTL); err != nil {
		return false
...
func (l *UserSliceLoader) PrimeManyNoReturn(keys []int, values [][]example.User) {
...
func (l *UserSliceLoader) PrimeMany(keys []int, values [][]example.User) []bool {
	...
	if l.redisConfig != nil {
		...
		if l.redisConfig.SetManyFunc != nil && len(keys) > 1 {
			...
			for i, key := range keys {
				kSet[i] = UserSliceLoaderCacheKeyPrefix + strconv.Itoa(key)
			}
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
...
func (l *UserSliceLoader) Clear(key int) error {
	if l.redisConfig != nil {
		// using Redis
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		err := l.redisConfig.DeleteFunc(ctx, UserSliceLoaderCacheKeyPrefix+strconv.Itoa(key))
		if err != nil {
			return err
		}
...
func (l *UserSliceLoader) ClearAllPrefix(prefix string) error {
	if l.redisConfig != nil {
		// using Redis
		if l.redisConfig.GetKeysFunc != nil {
			...
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
			keys, err := l.redisConfig.GetKeysFunc(ctx, UserSliceLoaderCacheKeyPrefix+prefix+"*")
...
func (l *UserSliceLoader) ClearAll() error {
	...
	if l.redisConfig != nil && len(b.data) == len(b.errors) && len(b.data) > 0 {
		...
		if len(b.errors) > 1 && l.redisConfig.SetManyFunc != nil {
			...
			for i, key := range b.keys {
				if b.errors[i] == nil {
					// convert keys slice (of int) to string slice
					kSet = append(kSet, UserSliceLoaderCacheKeyPrefix+strconv.Itoa(key))
