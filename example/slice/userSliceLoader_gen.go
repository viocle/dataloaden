...
func (l *UserSliceLoader) Clear(key int) {
	if l.redisConfig != nil {
		...
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		l.redisConfig.DeleteFunc(ctx, UserSliceLoaderCacheKeyPrefix+strconv.Itoa(key))
...
func (l *UserSliceLoader) ClearAllPrefix(prefix string) {
	if l.redisConfig != nil {
		// using Redis
		if l.redisConfig.GetKeysFunc != nil {
			...
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
			keys, _ := l.redisConfig.GetKeysFunc(ctx, UserSliceLoaderCacheKeyPrefix+prefix+"*")
