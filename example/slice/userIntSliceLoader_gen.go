...
func (l *UserIntSliceLoader) unsafeBatchSet() {
	if l.batch == nil {
		...
		clear(b.keysMap)
		clear(b.keys)
		l.batch = &userIntSliceLoaderBatch{now: 0, done: make(chan struct{}), keysMap: b.keysMap, keys: b.keys[:0], data: nil, errors: nil}
...
func (l *UserIntSliceLoader) createNewBatch() *userIntSliceLoaderBatch {
	return &userIntSliceLoaderBatch{now: 0, done: make(chan struct{}), keysMap: make(map[int]int, l.maxBatch), keys: make([]int, 0, l.maxBatch), data: nil, errors: nil}
}
...
func (l *UserIntSliceLoader) addToBatchUnsafe(key int) ([]*example.User, func() ([]*example.User, error)) {
	batch := l.batch
