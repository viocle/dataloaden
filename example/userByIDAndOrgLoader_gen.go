...
type UserByIDAndOrgLoaderConfig struct {
	...
	HookAfterClear func(key UserByIDAndOrg) error
	...
	if config.RedisConfig != nil {
		...
		if l.redisConfig.GetFunc != nil && l.redisConfig.SetFunc != nil && l.redisConfig.DeleteFunc != nil {
			...
			l.batchResultSet = func(key UserByIDAndOrg, value *User) {
				l.redisConfig.SetFunc(context.Background(), UserByIDAndOrgLoaderCacheKeyPrefix+l.redisConfig.KeyToStringFunc(key), value, l.redisConfig.SetTTL)
			}
			if l.redisConfig.KeyToStringFunc == nil {
				l.redisConfig.KeyToStringFunc = l.MarshalUserByIDAndOrgLoaderToString
			}
		}
	}
...
func (r *UserByIDAndOrgLoaderRedisConfig) Context() (context.Context, context.CancelFunc) {
...
func (l *UserByIDAndOrgLoader) Reset() {
	l.mu.Lock()
	defer l.mu.Unlock()
	...
	if l.redisConfig == nil {
		...
		l.cache = make(map[UserByIDAndOrg]*User)
...
func (l *UserByIDAndOrgLoader) unsafeBatchSet() {
	if l.batch == nil {
		...
		l.batch = &userByIDAndOrgLoaderBatch{loader: l, now: 0, done: make(chan struct{}), keysMap: b.keysMap, keys: b.keys[:0], data: nil, errors: nil, reqCount: 0, checkedIn: 0, lock: sync.Mutex{}}
...
func (l *UserByIDAndOrgLoader) createNewBatch() *userByIDAndOrgLoaderBatch {
	return &userByIDAndOrgLoaderBatch{
		loader:    l,
		now:       0,
		done:      make(chan struct{}),
		keysMap:   make(map[UserByIDAndOrg]int, l.maxBatch),
		keys:      make([]UserByIDAndOrg, 0, l.maxBatch),
		data:      nil,
		errors:    nil,
		lock:      sync.Mutex{},
		reqCount:  0,
...
func (l *UserByIDAndOrgLoader) LoadThunk(key UserByIDAndOrg) (*User, func() (*User, error)) {
	if l.redisConfig != nil {
		...
		defer cancel()
		v, err := l.redisConfig.GetFunc(ctx, UserByIDAndOrgLoaderCacheKeyPrefix+l.redisConfig.KeyToStringFunc(key))
		if err == nil {
...
func (l *UserByIDAndOrgLoader) unsafeAddToBatchNoLock(key UserByIDAndOrg) (*User, func() (*User, error)) {
	l.unsafeBatchSet()
	batch := l.batch
...
func (l *UserByIDAndOrgLoader) unsafeAddToBatch(key UserByIDAndOrg) (*User, func() (*User, error)) {
...
func (l *UserByIDAndOrgLoader) LoadAll(keys []UserByIDAndOrg) ([]*User, []error) {
	...
	if l.redisConfig != nil && l.redisConfig.GetManyFunc != nil {
		...
		for idx, key := range keys {
			rKeys[idx] = UserByIDAndOrgLoaderCacheKeyPrefix + l.redisConfig.KeyToStringFunc(key)
		}
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		...
		if err != nil {
			...
			for i := range errors {
				errors[i] = ErrUserByIDAndOrgLoaderGetManyLength
			}
		} else {
...
func (l *UserByIDAndOrgLoader) redisPrime(key UserByIDAndOrg, value *User) bool {
	ctx, cancel := l.redisConfig.Context()
	defer cancel()
	if err := l.redisConfig.SetFunc(ctx, UserByIDAndOrgLoaderCacheKeyPrefix+l.redisConfig.KeyToStringFunc(key), value, l.redisConfig.SetTTL); err != nil {
		return false
...
func (l *UserByIDAndOrgLoader) PrimeMany(keys []UserByIDAndOrg, values []*User) []bool {
	...
	if l.redisConfig != nil {
		...
		if l.redisConfig.SetManyFunc != nil && len(keys) > 1 {
			...
			for i, key := range keys {
				kSet[i] = UserByIDAndOrgLoaderCacheKeyPrefix + l.redisConfig.KeyToStringFunc(key)
			}
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
...
func (l *UserByIDAndOrgLoader) Clear(key UserByIDAndOrg) error {
	if l.redisConfig != nil {
		// using Redis
		ctx, cancel := l.redisConfig.Context()
		defer cancel()
		err := l.redisConfig.DeleteFunc(ctx, UserByIDAndOrgLoaderCacheKeyPrefix+l.redisConfig.KeyToStringFunc(key))
		if err != nil {
			return err
		}
...
func (l *UserByIDAndOrgLoader) ClearAllPrefix(prefix string) error {
	if l.redisConfig != nil {
		// using Redis
		if l.redisConfig.GetKeysFunc != nil {
			...
			ctx, cancel := l.redisConfig.Context()
			defer cancel()
			keys, err := l.redisConfig.GetKeysFunc(ctx, UserByIDAndOrgLoaderCacheKeyPrefix+prefix+"*")
...
func (l *UserByIDAndOrgLoader) ClearAll() error {
	...
	if l.redisConfig != nil && len(b.data) == len(b.errors) && len(b.data) > 0 {
		...
		if len(b.errors) > 1 && l.redisConfig.SetManyFunc != nil {
			...
			for i, key := range b.keys {
				if b.errors[i] == nil {
					// convert keys slice (of UserByIDAndOrg) to string slice
					kSet = append(kSet, UserByIDAndOrgLoaderCacheKeyPrefix+l.redisConfig.KeyToStringFunc(key))
...
func (b *userByIDAndOrgLoaderBatch) getResult(pos int) (*User, error) {
	var data *User
	b.lock.Lock()
	if pos < len(b.data) {
		data = b.data[pos]
